      ...
      <div className="space-y-6 w-full pb-20 relative">
        ...
        {activeTab === 'CATALOG' ? (
                          ...
                          <TableCell className="py-3">
                           ...
                           <TableCell className="text-right font-bold text-xs">
                             <span className={cn(
                               ...
                               {isCritical && (
                                 <span className={cn(
                                   "text-[8px] font-black uppercase px-1 py-0.5 rounded ml-1 block sm:inline-block border-none",
                                   isOut ? "bg-rose-100 dark:bg-rose-900/40 text-rose-600 dark:text-rose-300" : "bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-300"
                                 )}>
          ...
          <div className="space-y-8 animate-in fade-in duration-300 w-full">
            ...
            {/* 1. SECCIÓN DE PROVEEDORES ACTIVOS (GRILLA HORIZONTAL COMPACTA) */}
            <div className="space-y-3 w-full">
              <div className="flex justify-between items-center">
                ...
                <Button 
                  size="sm" 
                  variant="ghost" 
                  className="h-8 text-[11px] font-bold text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-slate-800 px-3 rounded-lg cursor-pointer"
                  onClick={() => setIsSupplierOpen(true)}
                >
                  <Plus className="h-4 w-4 mr-0.5" /> Registrar Proveedor
                </Button>
              </div>
              ...
              {suppliersLoading ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 w-full">
                  {Array.from({ length: 4 }).map((_, idx) => (
                    <div key={idx} className="border border-slate-200/60 dark:border-slate-800/80 rounded-xl p-3.5 bg-white dark:bg-slate-900 shadow-xs space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 w-full">
                  {suppliers.map((supplier) => (
                    <div 
                      key={supplier.id} 
                      className="border border-slate-200/60 dark:border-slate-800/80 rounded-xl p-3.5 bg-white dark:bg-slate-900 shadow-[0_4px_20px_rgba(0,0,0,0.015)] flex items-center justify-between gap-3 hover:border-slate-350 dark:hover:border-slate-700 transition-all"
                    >
                      <div className="min-w-0 flex-1">
                        <span className="text-xs font-black text-slate-800 dark:text-slate-100 block truncate">{supplier.name}</span>
                        <div className="flex flex-col gap-1 mt-1 text-[9px] text-slate-450 dark:text-slate-400 font-bold">
                          {supplier.phone && (
                            <span className="flex items-center gap-1 shrink-0"><Phone className="h-2.5 w-2.5 text-slate-455" /> {supplier.phone}</span>
                          )}
                          <span className="flex items-center gap-1 shrink-0"><FileText className="h-2.5 w-2.5 text-slate-455" /> {supplier._count?.purchases || 0} Facturas</span>
                        </div>
                      </div>
                      
                      <Button 
                        size="sm"
                        className="bg-emerald-500 hover:bg-emerald-600 text-black font-extrabold text-[10px] h-7 px-3 rounded-lg active:scale-95 transition-all shrink-0 cursor-pointer"
                ...
                <div className="text-center py-8 text-slate-400 text-xs bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-xl p-4 w-full">
                  No hay proveedores registrados.
                </div>
              )}
            </div>

            {/* 2. SECCIÓN DE BITÁCORA DE COMPRAS (TABLA A ANCHO COMPLETO) */}
            <div className="space-y-3 w-full">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
